...
if [[ "$rootsizedisk" -lt "100" ]]; then
    printf "${RED}FAIL${NC} - This machine has only %s GB of disk on the root partition${NC} \n" $rootsizedisk
...
yum install -y device-mapper-persistent-data lvm2 containerd.io docker-ce docker-ce-cli unzip kubelet kubeadm kubectl --disableexcludes=kubernetes
...
# Prepare the directories and binaries for backup/restore

mkdir -p /mnt/nfs/basebackup
mkdir -p /mnt/nfs/archived_backups
chmod 777 /mnt/nfs/basebackup
chmod 777 /mnt/nfs/archived_backups

yum install -y https://download.postgresql.org/pub/repos/yum/11/redhat/rhel-7-x86_64/postgresql11-libs-11.7-1PGDG.rhel7.x86_64.rpm
yum install -y https://download.postgresql.org/pub/repos/yum/11/redhat/rhel-7-x86_64/postgresql11-11.7-1PGDG.rhel7.x86_64.rpm
...
# Time to setup
printf "${PURPLE}COMPLETE${NC} Kubernetes cluster is almost ready to use. \n"
ELAPSEDMIN="$((($SECONDS / 60) % 60))min"
ELAPSEDSEC="$(($SECONDS % 60))sec"
printf "Operation time: %s %s" $ELAPSEDMIN $ELAPSEDSEC
sleep 2
echo ""
echo ""
